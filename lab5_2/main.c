...
uint32_t blue_delay_count = 0,
         red_delay_count = 0,
				 yellow_delay_count = 0,
				 green_delay_count = 0;
...
void parse_command (void) {
	...
	if (!strncmp((const char *)command_str, "set_state", strlen("set_state"))) {
		command_str += strlen("set_state") + 1;
		sf_state = conv_led_time_to_digit ((const char *)command_str);
		
		if ((sf_state > 0) & (sf_state < 4)) {
			traffic_light_delay = 0;
			traffic_light_state = sf_state - 1;
		} else {
			return_error();
			return;
		}
